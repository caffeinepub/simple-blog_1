...
import { useLikePost, useDislikePost, useGetPostReactions } from '../hooks/useQueries';
...
export default function ReactionButtons({ post }: ReactionButtonsProps) {
  ...
  const { data: reactions, isLoading: reactionsLoading } = useGetPostReactions(post.id);
  const likeMutation = useLikePost();
  const dislikeMutation = useDislikePost();

  const [likeParticles, setLikeParticles] = useState<Particle[] | null>(null);
  const [dislikeParticles, setDislikeParticles] = useState<Particle[] | null>(null);
  ...
  const hasLiked = isAuthenticated && post.likedBy.some((p) => p.toString() === userPrincipal);
  const hasDisliked = isAuthenticated && post.dislikedBy.some((p) => p.toString() === userPrincipal);

  const likeCount = reactions ? Number(reactions.likes) : post.likedBy.length;
  const dislikeCount = reactions ? Number(reactions.dislikes) : post.dislikedBy.length;
    ...
    <div className="flex items-center gap-3">
      ...
      <div className="relative">
        ...
        >
          ...
          />
          <span className={`tabular-nums transition-all duration-200 ${reactionsLoading ? 'opacity-40' : ''}`}>
            {likeCount}
          </span>
        </button>
      </div>
